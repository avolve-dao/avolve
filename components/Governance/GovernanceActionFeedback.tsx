...
function showFeedback(type: FeedbackType, message: string, duration: number = 4000) {
  const { toast } = useToast();
  toast({
    title: message,
    description: (
      <div
        className={`flex items-center gap-x-2 bg-gradient-to-r ${getGradient(type)} text-white rounded-md px-2 py-1`}
      >
        {getIcon(type)}
        <span>{message}</span>
      </div>
