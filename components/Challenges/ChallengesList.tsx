...
const ChallengesList: React.FC<unknown> = () => {
  const { challenges: challengesHook, loading, claimReward } = useChallenges();
      ...
      {loading ? (
        ...
        <>
          ...
          {challengesHook && challengesHook.length > 0 && (
            <div className="mb-6">
              <div
                className={`p-4 rounded-lg text-white ${getTokenColor(challengesHook[0].reward_token)}`}
              >
                  ...
                  <div>
                    {challengesHook[0].reward_token && (
                      <div className="text-right">
                        ...
                        <p className="text-xl font-bold">{challengesHook[0].progress} pts</p>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex items-center">
                        <div
                          className={`w-4 h-4 rounded-full mr-2 ${getTokenColor(challenge.reward_token)}`}
                        ></div>
                    ...
                    <div className="flex justify-between items-center">
                      <div className="w-full bg-gray-200 rounded-full h-2 mr-4">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, (challenge.progress / challenge.total_required) * 100)}%`,
                          }}
                        ></div>
                      </div>
                      <div className="flex items-center">
                        {isCompleted ? (
                          <span className="text-sm text-green-600 font-medium mr-2">Completed</span>
                        ) : (
                          <button
