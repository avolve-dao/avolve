...
export class ChallengeService {
   ...
   */
  public async getUserChallengeStreaks(
    userId: string
  ): Promise<ChallengeResult<ChallengeStreak[]>> {
   ...
   */
  public async getUserChallengeStreakByToken(
    userId: string,
    tokenType: string
  ): Promise<ChallengeResult<ChallengeStreak>> {
  ...
  public async getDailyChallengesWithStatus(
    ...
    try {
      ...
      const { data: challenges, error: challengesError } =
        await this.repository.getDailyChallengesByDay(dayOfWeek);
      ...
      return {
        ...
        error: new ChallengeError(
          'An unexpected error occurred while getting daily challenges with status',
          error
        ),
      };
    }
  ...
  public async getWeeklyChallengesWithStatus(
    ...
    try {
      ...
      const { data: challenges, error: challengesError } =
        await this.repository.getActiveWeeklyChallenges();
      ...
      return {
        data: null,
        error: new ChallengeError(
          'An unexpected error occurred while getting weekly challenges with status',
          error
        ),
      };
    }
  ...
  public async getChallengeDashboard(
    userId: string
  ): Promise<ChallengeResult<ChallengeDashboardData>> {
    try {
      const today = new Date();
      const dayOfWeek = today.getDay();

      // Get daily challenges with status
      const { data: dailyChallenges, error: dailyError } = await this.getDailyChallengesWithStatus(
        userId,
        dayOfWeek
      );
      ...
      const { data: weeklyChallenges, error: weeklyError } =
        await this.getWeeklyChallengesWithStatus(userId);
      ...
      // Get user streaks
      const { data: streaksArray, error: streaksError } =
        await this.getUserChallengeStreaks(userId);
      ...
      return {
        data: null,
        error: new ChallengeError(
          'An unexpected error occurred while getting challenge dashboard',
          error
        ),
      };
    }
  ...
  public async completeDailyChallenge(
    ...
    try {
      ...
      const { data: challenge, error: challengeError } =
        await this.repository.getDailyChallengeById(challengeId);
      ...
      const { data: completion, error: completionError } =
        await this.repository.completeDailyChallenge(userId, challengeId);
      ...
      const { data: mintResult, error: mintError } = await this.tokenService.mintTokens({
        userId,
        tokenId,
        amount: totalReward || challenge.reward_amount,
        reason: 'Daily challenge completion: ' + challenge.challenge_name,
      });
      ...
      return {
        data: null,
        error: new ChallengeError(
          'An unexpected error occurred while completing daily challenge',
          error
        ),
      };
    }
  ...
  public async updateWeeklyChallengeProgress(
    ...
    try {
      // Get the challenge details
      const { data: challenge, error: challengeError } =
        await this.repository.getWeeklyChallengeById(challengeId);
      ...
      // Update progress
      const { data: updatedProgress, error: progressError } =
        await this.repository.updateWeeklyChallengeProgress(
          userId,
          challengeId,
          progress,
          isCompleted
      ...
      if (isCompleted && updatedProgress.is_completed && !updatedProgress.reward_claimed) {
        // Calculate reward with streak bonus
        const { data: totalReward, error: rewardError } =
          await this.repository.calculateStreakBonus(
            userId,
            challenge.token_type,
            challenge.reward_amount,
            false // is weekly
        ...
        const { data: mintResult, error: mintError } = await this.tokenService.mintTokens({
          ...
          tokenId,
          amount: totalReward || challenge.reward_amount,
          reason: 'Weekly challenge completion: ' + challenge.challenge_name,
        });
      ...
      return {
        data: null,
        error: new ChallengeError(
          'An unexpected error occurred while updating weekly challenge progress',
          error
        ),
      };
    }
