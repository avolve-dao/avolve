...
const Post = memo(function Post({ post, currentUserId, onLike }: PostProps) {
  ...
  }, [post.id, post.liked, post.user.id, post.user.name, post.content, currentUserId, onLike, haptics])
    ...
    <Card className="overflow-hidden transition-all hover:shadow-md" ref={postRef}>
      <CardHeader className="p-4 pb-0">
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <Link href={`/dashboard/profile/${post.user.id}`} className="font-medium hover:underline">
                {post.user.name}
              </Link>
      ...
      <CardContent className="p-4">
        <div className="space-y-4">
          ...
          {post.media.length > 0 && (
            <div className={cn("grid gap-2 rounded-lg overflow-hidden", post.media.length > 1 && "grid-cols-2")}>
      ...
      <CardFooter className="p-4 pt-0 flex flex-col">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center space-x-2">
            <Button variant="ghost" size="sm" className="flex items-center space-x-1 h-auto px-2" onClick={handleLike}>
