...
// Create the context
const NotificationContext = createContext<ReturnType<typeof useNotifications> | undefined>(
  undefined
);
...
export function NotificationList({
  ...
  const { notifications, isLoading, markAsRead, loadNotifications } = useNotificationContext();
  ...
  if (notifications.length === 0) {
    return <div className={`p-4 text-center text-gray-500 ${className}`}>{emptyMessage}</div>;
  }
