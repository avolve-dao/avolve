...
export class AccessControlTester {
  ...
  constructor(supabaseUrl: string, supabaseAnonKey: string, adminServiceRoleKey: string) {
  ...
  async createTestUsers() {
    try {
      ...
      const roles = ['subscriber', 'participant', 'contributor', 'associate', 'builder', 'partner'];
      ...
      for (const role of roles) {
        ...
        // Assign role
        const { data: roleData, error: roleError } = await this.adminSupabase.rpc('assign_role', {
          p_user_id: userId,
          p_role_name: role,
        });
  ...
  async testUserProgressTracking() {
    try {
      ...
      const {
        ...
        password,
        id: userId,
      } = this.testUsers['participant'] || this.testUsers['subscriber'];
      ...
      // Check user progress
      const { data: progress, error: progressError } = await userClient.rpc(
        'get_all_pillars_progress'
      );

      if (progressError) throw progressError;

      // Check user experience phase
      const { data: phase, error: phaseError } = await userClient.rpc('get_user_experience_phase');
