...
export function usePermissions() {
  ...
  const permissionService = useMemo(
    () => new PermissionService(authService.getSupabaseClient()),
    [authService]
  );
  ...
  const hasPermissionViaToken = useCallback(
    async (resource: string, action: string): Promise<boolean> => {
      if (!user || !isAuthenticated) {
        return false;
      }
  ...
  const hasPermissionInGroup = useCallback(
  ...
  const getPermissionsInGroup = useCallback(
    async (groupId: string): Promise<Permission[]> => {
      try {
  ...
  const getTokenTypePermissions = useCallback(
    async (tokenTypeId?: string): Promise<TokenTypePermission[]> => {
      try {
  ...
  const getRolePermissions = useCallback(
    async (role: string): Promise<Permission[]> => {
      try {
  ...
  const createPermission = useCallback(
    async (resource: string, action: string, description?: string) => {
      try {
  ...
  const addPermissionToGroup = useCallback(
  ...
  const assignPermissionToTokenType = useCallback(
    async (tokenTypeId: string, permissionId: string, minBalance: number = 1) => {
      try {
        return await permissionService.assignPermissionToTokenType(
          tokenTypeId,
          permissionId,
          minBalance
        );
        ...
        return {
          data: null,
          error:
            error instanceof Error ? error : new Error('Failed to assign permission to token type'),
        };
      }
    },
    [permissionService]
  ...
  const assignRoleToUser = useCallback(
    async (userId: string, role: string, grantedBy?: string, expiresAt?: Date) => {
      try {
  ...
  const assignPermissionToRole = useCallback(
    async (role: string, resource: string, action: string) => {
      try {
