...
const ROLE_DETAILS: Record<string, { label: string; color: string; gradient: string; icon: string; description: string }> = {
  subscriber: {
    label: 'Subscriber',
    color: '#A8A8A8',
...
export default function RoleProgressionCard() {
  const [roles, setRoles] = useState<{ role_type: string; points: number; last_action_at: string }[]>([]);
    ...
    <div className="card" style={{padding: '1.5rem', borderRadius: 12, background: '#f9f9fc', boxShadow: '0 2px 8px #e0e0e0'}}>
      ...
      <ul style={{listStyle: 'none', padding: 0}}>
        {['subscriber', 'participant', 'contributor'].map(roleType => {
          const role = roles.find(r => r.role_type === roleType) || { points: 0, last_action_at: '' };
          const details = ROLE_DETAILS[roleType];
          return (
            <li key={roleType} style={{marginBottom: 18, background: details.gradient, borderRadius: 8, padding: '0.75rem 1rem', display: 'flex', alignItems: 'center'}}>
              <span style={{fontSize: 28, marginRight: 16}}>{details.icon}</span>
              <div style={{flex: 1}}>
                <div style={{fontWeight: 600, fontSize: 18, color: details.color}}>{details.label}</div>
                <div style={{fontSize: 13, color: '#444', marginBottom: 4}}>{details.description}</div>
                <div style={{height: 8, background: '#eee', borderRadius: 4, margin: '6px 0'}}>
                  <div style={{width: `${Math.min(100, (role.points / maxPoints) * 100)}%`, height: 8, background: details.color, borderRadius: 4, transition: 'width 0.5s'}} />
                </div>
                <span style={{fontSize: 13, color: '#666'}}>Points: {role.points} {role.last_action_at && (<span style={{marginLeft: 8, color: '#888'}}>Last: {new Date(role.last_action_at).toLocaleString()}</span>)}</span>
              </div>
            </li>
          );
        })}
      </ul>
      <div style={{marginTop: 16, padding: 12, background: '#fffbe6', borderRadius: 8, color: '#b8860b', fontWeight: 500}}>
        <span role="img" aria-label="Genie">🧞‍♂️</span> Next step: {nextAction}
      </div>
    </div>
  );
}
