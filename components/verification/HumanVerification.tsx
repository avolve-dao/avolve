...
export function HumanVerification({ 
  ...
  const handleChallengeComplete = async (points: number, challengeType: string, challengeData: any) => {
    try {
      const supabase = createClient()
  ...
  return (
    <Card className="w-full">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl font-bold">Human Verification</CardTitle>
        <CardDescription>
          Complete interactive challenges to verify your account
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="mb-4">
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-sm font-medium">{currentScore}/{requiredScore}</span>
          </div>
      ...
      <CardFooter className="pt-0">
        <p className="text-xs text-muted-foreground">
          Complete different types of challenges to earn verification points. You need {requiredScore} points to verify your account.
        </p>
      </CardFooter>
    </Card>
  )
