...
export async function mintOnboardingTokens() {
  ...
  try {
    ...
    // TODO: Review type safety for supabase calls
    const { error: sapMintError } = await (supabase as any).rpc('process_token_transaction', {
      p_from_user_id: null,
    ...
    // TODO: Review type safety for supabase calls
    const { error: pspMintError } = await (supabase as any).rpc('process_token_transaction', {
      p_from_user_id: null,
      p_to_user_id: user.id,
