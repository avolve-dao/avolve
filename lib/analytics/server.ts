...
export async function getUserEngagementMetrics(userId: string) {
  ...
  const sessionDurations = recentLogins.map(session => {
    ...
    const end = session.ended_at ? new Date(session.ended_at).getTime() : start + 10 * 60 * 1000; // Default to 10 minutes if no end time
...
function calculatePhaseCompletionDate(userData: UserBehaviorData, phase: string): string | null {
  ...
  const loginAdjustment = Math.min(
    Math.max(userData.engagementMetrics.loginFrequency / 0.5, 0.5),
    2
  );
...
async function getMilestoneRecommendations(userData: UserBehaviorData) {
  ...
  const currentPhaseProgress =
    userData.phaseProgress[userData.currentPhase as keyof typeof userData.phaseProgress];
