...
function SidebarNavItem({ route, active, expanded, onExpand, depth = 0 }: SidebarNavItemProps) {
  ...
  return (
    <div
      className={cn(
        'flex flex-col group/sidebar-item',
        depth > 0 && 'ml-6 border-l border-muted/30 pl-3'
      )}
      ...
      <div className="flex items-center group">
        <Link
          ...
          className={cn(
            ...
            active
              ? `bg-gradient-to-r ${route.gradientClass} text-white shadow-lg scale-[1.03]`
              : 'text-zinc-700 dark:text-zinc-200',
            depth === 0 &&
              'transition-transform duration-200 group-hover/sidebar-item:scale-[1.04] group-focus-within/sidebar-item:scale-[1.04]'
          )}
          aria-current={active ? 'page' : undefined}
          tabIndex={0}
        ...
        >
          {icon && (
            <span
              className={cn(
                ...
                active
                  ? 'scale-110'
                  : 'group-hover/sidebar-item:scale-110 group-focus-visible/sidebar-item:scale-110'
              )}
            >
              {icon}
            </span>
          ...
          <span
            className="block w-2 h-8 rounded-full mr-2"
        ...
        {hasChildren && (
          <button
            ...
            className={cn(
              'ml-2 p-1 rounded focus-visible:ring-2 focus-visible:ring-primary/60 transition-colors',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-xl p-8 max-w-sm w-full relative animate-fade-in">
        ...
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">
          Explore the sidebar to navigate through all the main journeys. Tap the menu button on
          mobile to open the sidebar. Click any route to see its puzzle pieces. Enjoy your journey!
          🚀
        </p>
        <button
...
export function AppSidebar({ activeFocus, className, ...props }: AppSidebarProps) {
  ...
  const findActiveMainRoute = (): NavItem => {
    ...
    return (
      avolveData.navMain.find((r: NavItem) => r.id === seg || r.href.replace('/', '') === seg) ||
      avolveData.navMain[0]
    );
  };
    ...
    <SidebarProvider>
      ...
      <Sidebar
        ...
        className={cn(
          ...
          className,
          isMobile
            ? 'fixed top-0 left-0 h-full w-[18rem] transition-transform duration-300 ease-in-out'
            : ''
        )}
        {...props}
      >
