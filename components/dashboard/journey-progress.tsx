...
export function JourneyProgress({ userId }: JourneyProgressProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const { data: regenAnalytics } = await supabase.rpc('get_user_regen_analytics', {
  ...
  async function fetchTokenBalances() {
    ...
    const { data: tokens } = await supabase.from('tokens').select('id, symbol');
              ...
              {recentTokens.length > 0 ? (
                recentTokens.map((token, index) => (
                  ...
                  >
                    <div className="flex items-center">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center ${getTokenColor(token.tokens?.token_type || '')}`}
                      >
                    ...
                    <Badge
                      variant="outline"
                      className={getTokenColor(token.tokens?.token_type || '')}
                    >
                      +{token.amount}
                    </Badge>
                  </motion.div>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="font-medium">
                        {streakData.current_daily_streak} Day Streak
                      </div>
