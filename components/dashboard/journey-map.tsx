...
export function JourneyMap({ userId }: JourneyMapProps) {
  ...
  const calculateOverallProgress = () => {
    ...
    userProgress.forEach(pillar => {
      const phaseIndex = ['discover', 'onboard', 'scaffold', 'endgame'].indexOf(
        pillar.current_phase
      );
            ...
            <TabsList className="grid grid-cols-3 mb-4">
              <TabsTrigger value="superachiever" className="relative">
                ...
                {userProgress.find(p => p.pillar === 'superachiever')?.current_phase && (
                  <span
                    className={`absolute -top-1 -right-1 w-3 h-3 rounded-full ${getPhaseColor(getCurrentPhase('superachiever'))}`}
                  ></span>
                )}
              </TabsTrigger>
              <TabsTrigger value="superachievers" className="relative">
                ...
                {userProgress.find(p => p.pillar === 'superachievers')?.current_phase && (
                  <span
                    className={`absolute -top-1 -right-1 w-3 h-3 rounded-full ${getPhaseColor(getCurrentPhase('superachievers'))}`}
                  ></span>
                )}
              </TabsTrigger>
              <TabsTrigger value="supercivilization" className="relative">
                ...
                {userProgress.find(p => p.pillar === 'supercivilization')?.current_phase && (
                  <span
                    className={`absolute -top-1 -right-1 w-3 h-3 rounded-full ${getPhaseColor(getCurrentPhase('supercivilization'))}`}
                  ></span>
                )}
              </TabsTrigger>
            </TabsList>
                ...
                <div className="space-y-4">
                  ...
                  <div className="mb-4">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-xs">{getPhaseProgress(pillar)}%</span>
                    </div>
                    <Progress value={getPhaseProgress(pillar)} className="h-2" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {getAvailableMilestones(pillar).length === 0 ? (
                      ...
                      getAvailableMilestones(pillar)
                        ...
                        .map((milestone, index) => {
                          const { isCompleted, inProgress, progress } =
