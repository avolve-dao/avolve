...
export default function OnboardingStatusTable() {
  ...
  const allSteps = Array.from(new Set(users.flatMap(u => u.completed_steps.concat(u.current_step || [])))).filter(Boolean);
  ...
  let filteredUsers = users.filter(user => {
    ...
    const statusMatch = !filter.status || (filter.status === "complete" ? !!user.completed_at : !user.completed_at);
  ...
  async function handleMarkComplete(user: UserOnboarding) {
    if (!window.confirm("Are you sure you want to mark this user's onboarding as complete?")) return;
    try {
  ...
  return (
    <div className="overflow-x-auto" aria-live="polite">
      {showCelebration && (
        <AdminCelebrationToast show={showCelebration} userName={celebratedUser} onClose={() => setShowCelebration(false)} />
      )}
      <div className="flex flex-wrap gap-4 mb-2" role="region" aria-label="Filters">
        <label htmlFor="filter-step" className="flex items-center" title="Filter users by their current onboarding step">
          Step:
          <select
            id="filter-step"
            value={filter.step}
        ...
        <label htmlFor="filter-role" className="flex items-center" title="Filter users by their role">
          Role:
          <select
            id="filter-role"
            value={filter.role}
        ...
        <label htmlFor="filter-status" className="flex items-center" title="Filter users by onboarding completion status">
          Status:
          <select
            id="filter-status"
            value={filter.status}
        ...
        <span className="text-gray-500 text-xs ml-4" aria-live="polite">Use Tab to navigate filters. Press Enter to apply.</span>
      </div>
      <table className="min-w-full border border-gray-300 rounded shadow" aria-label="Onboarding status table">
        <thead>
          <tr className="bg-gray-100">
            {[
              ...
              { label: "Completed Steps", key: "completed_steps", tooltip: "All completed onboarding steps" },
              { label: "Completed At", key: "completed_at", tooltip: "When onboarding was completed" },
              { label: "Support Admin", key: "support_admin_name", tooltip: "Support admin assigned to this user" },
              { label: "Marked Complete By", key: "marked_complete_admin_name", tooltip: "Admin who marked onboarding complete" },
              { label: "Actions", key: "actions", tooltip: "Admin actions for this user", isAction: true },
            ].map(col => (
              <th
                key={col.key}
                ...
                onClick={() => {
                  if (!col.isAction) {
                    ...
                    else { setSortBy(col.key as keyof UserOnboarding); setSortAsc(true); }
                  }
                }}
                tabIndex={0}
        ...
        <tbody>
          {filteredUsers.map(user => (
            <tr key={user.id} className="hover:bg-blue-50 focus-within:bg-blue-100" tabIndex={0}>
              ...
              <td className="py-2 px-4 border-b">{user.date_joined ? new Date(user.date_joined).toLocaleDateString() : "-"}</td>
      ...
      <div className="text-gray-600 text-xs mt-2" aria-live="polite">
        Table is keyboard navigable. Use Tab/Shift+Tab to move between filters, headers, and actions. Use Enter to sort columns or activate buttons.
      </div>
    </div>
  );
}
