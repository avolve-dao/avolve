          ...
          rewards.map((reward) => (
            <Card key={reward.id} className="overflow-hidden">
              <div className={`h-2 w-full ${
                reward.reward_type === 'achievement' ? 'bg-gradient-to-r from-amber-400 to-yellow-500' :
                reward.reward_type === 'contribution' ? 'bg-gradient-to-r from-teal-400 to-cyan-500' :
                reward.reward_type === 'referral' ? 'bg-gradient-to-r from-violet-500 to-purple-600' :
                reward.reward_type === 'daily' ? 'bg-gradient-to-r from-blue-400 to-indigo-500' :
              ...
              <CardHeader className="pb-2">
                <div className="flex justify-between items-center">
                  ...
                  <div className={`p-1.5 rounded-full ${
                    reward.reward_type === 'achievement' ? 'bg-amber-100 text-amber-700' :
                    reward.reward_type === 'contribution' ? 'bg-teal-100 text-teal-700' :
                    reward.reward_type === 'referral' ? 'bg-violet-100 text-violet-700' :
                    reward.reward_type === 'daily' ? 'bg-blue-100 text-blue-700' :
              ...
              <CardContent>
                ...
                <div className="flex justify-between items-center mb-1 text-sm">
                  <span>Reward</span>
                  <span className="font-medium">{reward.amount} {reward.token_symbol}</span>
                </div>
                ...
                {reward.requirements.progress && (
                  <>
                    <div className="flex justify-between items-center text-xs text-muted-foreground mb-1">
                      <span>Progress</span>
                      <span>{reward.requirements.progress.current}/{reward.requirements.progress.required}</span>
                    </div>
              ...
              <CardFooter>
                ...
                >
                  {claimingId === reward.id ? 'Claiming...' : 
                   reward.is_claimed ? 'Claimed' : 
                   !reward.can_claim ? 'Not Available' : 
                   'Claim Reward'}
                </Button>
              </CardFooter>
            </Card>
