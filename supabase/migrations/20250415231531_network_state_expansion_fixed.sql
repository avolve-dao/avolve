-- Migration: Network State Expansion for Avolve (Fixed)
-- Purpose: Ensure all columns exist before creating views; support upgrades from older schemas
-- Author: Cascade AI
-- Date: 2025-04-15 23:15 UTC

/*
  This migration will:
    1. Create or alter teams table for all required columns
    2. Create/expand all related tables
    3. Add network state metadata and progression logic
    4. Implement DAO/coop proposal & voting logic
    5. Add on-chain census views
    6. Ensure RLS and security best practices
*/
...
alter table public.teams add column if not exists type text default 'union';
alter table public.teams add column if not exists status text default 'active';
alter table public.teams add column if not exists milestone_level integer default 0;
alter table public.teams add column if not exists milestone_desc text;
