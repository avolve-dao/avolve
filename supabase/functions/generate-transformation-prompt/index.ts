...
serve(async (req) => {
  try {
    ...
    if (!journeyType || !PROMPT_TEMPLATES[journeyType as keyof typeof PROMPT_TEMPLATES]) {
      return new Response(
        JSON.stringify({ error: 'Invalid journey type' }),
        { status: 400 }
      )
    }

    const basePrompt = PROMPT_TEMPLATES[journeyType as keyof typeof PROMPT_TEMPLATES][
      Math.floor(Math.random() * PROMPT_TEMPLATES[journeyType as keyof typeof PROMPT_TEMPLATES].length)
    ]
