...
export class TeamsService {
  ...
  async createTeam(
    ...
    try {
      ...
      await this.recordTeamMetric(teamId, 'growth', 1, {
        action: 'team_created',
        creator_id: ownerId,
      });
  ...
  async joinTeam(
    ...
    try {
      ...
      // Join the team
      const { error: joinError } = await this.client.from('team_members').insert({
        team_id: teamId,
        user_id: userId,
  ...
  async leaveTeam(
    ...
    try {
      ...
      // Record team metric
      await this.recordTeamMetric(teamId, 'growth', -1, { action: 'member_left', user_id: userId });
  ...
  async transferLeadership(
    ...
    try {
      ...
      // Record team metric
      await this.recordTeamMetric(teamId, 'interaction', 1, {
  ...
  private async recordTeamMetric(
    ...
    try {
      const { error } = await this.client.from('team_metrics').insert({
        team_id: teamId,
  ...
  async startTeamChallenge(
    ...
    try {
      ...
      // Start the challenge
      const { error: startError } = await this.client.from('team_challenges').insert({
        team_id: teamId,
  ...
  async completeTeamChallenge(
    ...
    try {
      ...
      // Record team metric
      await this.recordTeamMetric(teamId, 'interaction', 5, {
