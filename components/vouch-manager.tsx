          ...
          <Tabs defaultValue="search" value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="history" className="mt-2">
              {loading ? (
                ...
                <div className="space-y-2">
                  {vouches.map((vouch) => (
                    <div key={vouch.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        <Avatar>
                          <AvatarImage src={vouch.vouched_user_avatar} />
                          <AvatarFallback>
                            {vouch.vouched_user_name?.charAt(0) || vouch.vouched_user_email?.charAt(0) || 'U'}
                          </AvatarFallback>
                        </Avatar>
                        <div>
                          <p className="font-medium">{vouch.vouched_user_name || 'Unnamed User'}</p>
                          <p className="text-sm text-muted-foreground">{vouch.vouched_user_email}</p>
                          <p className="text-xs text-muted-foreground mt-1">
                            Vouched {formatDistanceToNow(new Date(vouch.created_at), { addSuffix: true })}
                          </p>
                        </div>
                      </div>
