...
export function useTokens() {
  ...
  const fetchAllTokens = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: fetchError } = await supabase.from('tokens').select('*').order('symbol');
  ...
  const fetchUserTransactions = useCallback(
    async (limit = 10, offset = 0) => {
      ...
      try {
        ...
        const { data, error: fetchError } = await supabase.rpc('get_user_transactions', {
          p_user_id: user.id,
          p_limit: limit,
          p_offset: offset,
        });
  ...
  const transferTokens = useCallback(
    async (
      ...
      try {
        ...
        const { data, error: transferError } = await supabase.rpc('transfer_tokens', {
   ...
   */
  const spendGenTokens = useCallback(
    async (
      amount: number,
      ...
      try {
        ...
        const { data, error: spendError } = await supabase.rpc('spend_gen_tokens', {
          p_user_id: user.id,
          p_amount: amount,
          p_purpose: purpose,
        });
  ...
  const getTokenDetails = useCallback(
    async (symbol: string): Promise<Token | null> => {
  ...
  const hasEnoughTokens = useCallback(
    (symbol: string, amount: number): boolean => {
  ...
  const getTokenSupply = useCallback(
    async (symbol: string) => {
      ...
      try {
        const { data, error: fetchError } = await supabase.rpc('get_token_supply_stats', {
          p_symbol: symbol,
        });
  ...
  const getUserToken = useCallback(
    async (tokenId: string): Promise<any | null> => {
  ...
  const getTokenBalanceById = useCallback(
    async (tokenId: string): Promise<number | null> => {
  ...
  const getUserTokenBalance = useCallback(
    async (tokenCode: string): Promise<{ data: number; error: any }> => {
      if (!supabase || !user) return { data: 0, error: 'Not authenticated' };
      const { data: tokenTypes, error: tokenTypesError } = await supabase
        .from('token_types')
        .select('*');
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: {
          new?: { token_id: string; balance: number };
          old?: { token_id: string; balance: number };
        }) => {
