...
export function ImageVerification({ onComplete, isLoading = false }: ImageVerificationProps) {
  ...
  const handleSubmit = () => {
    ...
    const targetImageIds = currentChallenge.images
    ...
    const accuracy = (totalImages - (incorrectSelections.length + missedTargets.length)) / totalImages
      ...
      <CardContent>
        ...
        {result === 'failure' && (
          <p className="text-sm text-red-500 text-center">
            Incorrect selections. Try again.
          </p>
        )}
      </CardContent>
      <CardFooter>
