...
const DebugPanel = dynamic(
  () => import('@/components/debug/debug-panel').then(mod => mod.default),
  {
    ssr: false,
    loading: () => (
      <div className="p-4 text-center text-sm text-gray-500">Loading debug tools...</div>
    ),
  }
);
...
const ChallengeDashboard: React.FC<ChallengeDashboardProps> = () => {
  ...
  const claimDailyToken = async () => {
    ...
    try {
      ...
      // Log the completion
      const { error: completionError } = await supabase.from('challenge_completions').insert({
        user_id: userId,
          ...
          <AnimatePresence>
            {showDebugPanel && (
              ...
              >
                <Suspense
                  fallback={
                    <div className="p-4 bg-white shadow rounded">Loading debug tools...</div>
                  }
                >
                  <DebugPanel data={debugData} onClose={() => setShowDebugPanel(false)} />
                </Suspense>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400">
            Complete daily challenges to earn tokens and build your streak
          </p>
        </div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Earn {todayTokenSymbol} tokens and increase your streak
                  </CardDescription>
                </div>
              ...
              {loading ? (
                ...
                <div className="text-center py-8">
                  <p className="text-muted-foreground mb-4">
                    No active challenge today. Check back later!
                  </p>
                </div>
              ) : (
                <>
                  <div className="bg-muted p-4 rounded-lg">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <span>
                          Reward: {Math.round(todayChallenge.token_reward * getStreakMultiplier())}{' '}
                          {todayTokenSymbol}
                  ...
                  <div>
                    ...
                    <Progress
                      value={Math.min((todayTokenBalance / 100) * 100, 100)}
                      className="h-2"
                    />
                  </div>
    ...
    <div className="text-center text-sm text-gray-500 dark:text-gray-400">
      <div className="flex items-center justify-center">
        ...
        {daysToNextMilestone > 0 ? (
          <span>
            {daysToNextMilestone} day{daysToNextMilestone > 1 ? 's' : ''} until next bonus level
          </span>
        ) : (
    ...
    <div className="relative w-32 h-32">
      <svg viewBox="0 0 100 100" className="w-full h-full">
        ...
        <text
          ...
          fontSize="12"
          fontWeight="bold"
          fill={highlightThree ? '#22c55e' : '#94a3b8'}
        >
          3
        </text>
        <text
          ...
          fontSize="12"
          fontWeight="bold"
          fill={highlightSix ? '#a855f7' : '#94a3b8'}
        >
          6
        </text>
        <text
          ...
          fontSize="12"
          fontWeight="bold"
          fill={highlightNine ? '#3b82f6' : '#94a3b8'}
        >
          9
        </text>
