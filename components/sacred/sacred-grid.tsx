...
export function SacredGrid({
  ...
  const getGridStyle = () => {
    switch (variant) {
      case 'golden':
        ...
        if (columns === 2)
          return { gridTemplateColumns: `${PHI_INVERSE * 100}% ${(1 - PHI_INVERSE) * 100}%` };
        if (columns === 3)
          return {
            gridTemplateColumns: `${PHI_INVERSE * PHI_INVERSE * 100}% ${PHI_INVERSE * 100}% ${(1 - PHI_INVERSE - PHI_INVERSE * PHI_INVERSE) * 100}%`,
          };
      ...
      case 'phi':
        ...
        const phiColumns = phiWidths.map(width => `${(width / phiTotal) * 100}%`).join(' ');
  ...
  return (
    <div
      className={cn('grid w-full', gapMap[gap], getBackgroundPattern(), className)}
      style={getGridStyle()}
      {...props}
    >
      ...
      {React.Children.map(children, (child, index) => {
        ...
        const childStyle = variant === 'phi' && index === 0 ? { aspectRatio: `${PHI} / 1` } : {};
