...
const testInvitationFlow = async () => {
  ...
  logResult(
    ...
    createResult.success ? `Code: ${createResult.data.code}` : createResult.data?.error
  );
  ...
  logResult(
    ...
    validateResult.success ? 'Valid invitation' : validateResult.data?.error
  );

  if (!validateResult.success) return false;

  console.log(
    chalk.yellow('ℹ️ Note: Full claim test requires a real browser session and is not automated')
  );
...
const testOnboardingFlow = async () => {
  ...
  logResult(
    ...
    updateStepResult.success ? 'Step updated' : updateStepResult.data?.error
  );
  ...
  logResult(
    ...
    getProgressResult.success,
    getProgressResult.success ? 'Progress retrieved' : getProgressResult.data?.error
  );
...
const testFeatureFlagSystem = async () => {
  ...
  logResult(
    'Get Features',
    getFeaturesResult.success,
    getFeaturesResult.success
      ? `${Object.keys(getFeaturesResult.data?.features || {}).length} features found`
      : getFeaturesResult.data?.error
  );
  ...
  logResult(
    ...
    checkFeatureResult.success,
    checkFeatureResult.success ? checkFeatureResult.data?.message : checkFeatureResult.data?.error
  );
  ...
  logResult(
    ...
    notificationResult.success ? 'Notification sent' : notificationResult.data?.error
  );
