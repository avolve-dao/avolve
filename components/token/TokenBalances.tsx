...
export function TokenBalances() {
  ...
  useEffect(() => {
    ...
    const fetchTokenData = async () => {
      // Fetch token types
      const { data: types } = await supabase.from('token_types').select('*').order('category');
  ...
  const getTokenBalance = (tokenId: string): TokenBalance => {
    return (
      balances.find(b => b.token_type_id === tokenId) || {
        ...
        return (
          ...
          >
            <Card className="p-4 relative overflow-hidden">
              <div
                className={`absolute inset-0 bg-gradient-to-r ${frequencyGradients[token.frequency]} opacity-10`}
              />
              ...
              <div className="relative z-10">
                ...
                <Progress value={stakedPercentage} className="h-2 mb-2" />

                <div className="text-xs text-gray-500 text-right">
                  {stakedPercentage.toFixed(1)}% Staked
                </div>

                <div className="mt-2 text-xs text-gray-400">Frequency: {token.frequency}</div>
              </div>
            </Card>
          </motion.div>
        );
