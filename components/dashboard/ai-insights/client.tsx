...
export function AIInsightsClient({
  ...
  const getHighlightedPrediction = () => {
    ...
    if (currentPhaseCompletion && currentPhaseCompletion !== 'Insufficient data') {
      const daysToCompletion = Math.round(
        (new Date(currentPhaseCompletion).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
      );
  ...
  return (
    <div className="space-y-4">
      {/* Main insight card */}
      <Card
        className={`border ${highlightedPrediction.color.split(' ')[2]} hover:shadow-md transition-shadow`}
      >
            ...
            >
              ...
              <div className="mb-4">
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span>
                    {regenScore} / {(regenLevel + 1) * 500}
                  </span>
                </div>
                <div className="relative">
                  <Progress value={(regenScore / ((regenLevel + 1) * 500)) * 100} className="h-2" />
                  {daysToNextLevel && (
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>
                            Level {regenLevel + 1} in ~{formatDays(daysToNextLevel)}
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {phaseCompletionPredictions[currentPhase] &&
          phaseCompletionPredictions[currentPhase] !== 'Insufficient data' && (
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {Object.entries(tokenPredictions)
                      ...
                      .map(
                        ([symbol, pred]) =>
                          `${pred.target} ${symbol} in ${formatDays(pred.daysToTarget)}`
                      )
                      .join(', ')}
                  </p>
                </div>
