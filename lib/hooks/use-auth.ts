...
const AuthContext = createContext<AuthContextType>({
  ...
  signUp: async (email: string, password: string, metadata?: Record<string, any>) =>
    await AuthService.getBrowserInstance().signUp(email, password, metadata),
  signOut: async () => await AuthService.getBrowserInstance().signOut(),
  resetPassword: async (email: string) =>
    await AuthService.getBrowserInstance().resetPassword(email),
  updatePassword: async (newPassword: string) =>
    await AuthService.getBrowserInstance().updatePassword(newPassword),
  ...
  resendConfirmationEmail: async (email: string) =>
    await AuthService.getBrowserInstance().resendConfirmationEmail(email),
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshUser = useCallback(async () => {
    try {
      ...
      if (sessionResult && sessionError) {
        const authErr =
          sessionError instanceof AuthError
            ? sessionError
            : new AuthError(sessionError?.message || 'Unknown error');
        setError(authErr);
        setIsLoading(false);
          ...
          if (profileError) {
            const authErr =
              profileError instanceof AuthError
                ? profileError
                : new AuthError(profileError?.message || 'Unknown error');
            setError(authErr);
            setIsLoading(false);
            return { data: null, error: authErr };
          }
        ...
        if (result && result.error) {
          const authErr =
            result.error instanceof AuthError
              ? result.error
              : new AuthError(result.error?.message || 'Unknown error');
          setError(authErr);
          setIsLoading(false);
          return { data: null, error: authErr };
        }
  ...
  const isMfaRequired = useCallback(async () => {
    const user =
      typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('user') || 'null') : null;
    if (!user || !user.id) return false;
    const result = await AuthService.getBrowserInstance().isMfaRequired(user.id);
  ...
  // Setup TOTP
  const setupTotp = useCallback(async (friendlyName?: string) => {
    const user =
      typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('user') || 'null') : null;
    if (!user || !user.id) return { data: null, error: new AuthError('User not found') };
    return await AuthService.getBrowserInstance().setupTotp(user.id, friendlyName);
  ...
  // Verify TOTP factor
  const verifyTotpFactor = useCallback(async (factorId: string, code: string) => {
    const user =
      typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('user') || 'null') : null;
    if (!user || !user.id) {
      return { data: null, error: new AuthError('User not found') };
  ...
  const generateRecoveryCodes = useCallback(async (count?: number) => {
    const user =
      typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('user') || 'null') : null;
    if (!user || !user.id) return { data: null, error: new AuthError('User not found') };
    return await AuthService.getBrowserInstance().generateRecoveryCodes(user.id, count);
