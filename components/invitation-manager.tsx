  ...
  const fetchJourneyStatus = async () => {
    try {
      ...
      const invitationLimit =
        level === 'full_member' ? 5 : level === 'contributor' ? 3 : level === 'vouched' ? 1 : 0;
      ...
      const usedInvitations = invitations.filter(
        inv => new Date(inv.created_at) > thirtyDaysAgo && inv.status !== 'expired'
      ).length;
        ...
        <div className="bg-muted p-3 rounded-lg text-sm">
          ...
          <div className="mt-2 text-xs text-muted-foreground">
            {canCreateInvitation ? (
              <p>
                You can invite{' '}
                {(journeyStatus?.invitation_limit || 0) - (journeyStatus?.used_invitations || 0)}{' '}
                more members this month
              </p>
            ) : (
              <p>
                You've reached your invitation limit for this month or need to progress to the next
                level
              </p>
            )}
          </div>
                        ...
                        <div className="flex gap-1">
                          ...
                          {invitation.email && (
                            <Button
                              ...
                              size="icon"
                              onClick={() =>
                                sendInvitationEmail(invitation.email!, invitation.code)
                              }
                            >
