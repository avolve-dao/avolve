...
const seededPosts: Post[] = [
  {
    ...
    avatar: 'F',
    content:
      'Welcome to Supercivilization! Share your intention for today and help us build the future.',
    reactions: 5,
      ...
      <Card className="mb-8 bg-gradient-to-br from-fuchsia-950/80 to-zinc-900/80 border-fuchsia-900">
        <CardHeader className="flex flex-row items-center gap-3 pb-2">
          <Image
            ...
            alt="Supercivilization Feed"
            width={24}
            height={24}
            className="drop-shadow-md"
          />
          <span className="text-xl font-bold text-fuchsia-200">Supercivilization Feed</span>
          <Badge className="ml-auto bg-fuchsia-700/80 text-xs" aria-label="Live feed">
            Live
          </Badge>
        </CardHeader>
        <CardContent>
          <div className="flex gap-2 mb-4">
            <input
              ...
              placeholder={
                realPosts.length > 0
                  ? 'Share your intention, win, or question...'
                  : 'Be the first Regen to post your intention and start the journey!'
              }
              value={input}
          ...
          <div className="space-y-4">
            {filteredPosts.map(post => (
              <Card
                ...
                role="article"
                aria-label={`Post by ${post.user}`}
              >
                <CardHeader className="flex flex-row items-center gap-3 pb-1">
                  <Image
                    src={
                      post.type === 'intention'
                        ? '/icons/icon-superachiever.svg'
                        : post.type === 'reflection'
                          ? '/icons/icon-supermind-superpowers.svg'
                          : '/icons/icon-superachievers.svg'
                    }
                    alt={post.user}
                    width={20}
                    height={20}
                  ...
                  <Badge className="ml-2 bg-fuchsia-800/70 text-xs" aria-label={post.badge}>
                    {post.badge}
                  </Badge>
                  {post.isNew && (
                    <Badge className="ml-2 bg-fuchsia-500 text-xs" aria-label="New post">
                      New!
                    </Badge>
                  )}
            ...
            {posts.some(post => !MVP_POST_TYPES.includes(post.type)) && (
              <div className="opacity-50 text-xs text-gray-400 mt-4">
                More post types coming soon…
              </div>
            )}
            {filteredPosts.length === 0 && (
              <div className="text-center text-zinc-400 text-sm py-10">
                <Image
                  ...
                  width={36}
                  height={36}
                  className="mx-auto mb-2 grayscale opacity-40"
                />
                <div>
                  No intentions posted yet. Be the first Regen to start the Supercivilization
                  journey!
                </div>
              </div>
            )}
