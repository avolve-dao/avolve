...
export function ChatMessageItem({ message, isOwnMessage, showHeader }: ChatMessageItemProps) {
  ...
  return (
    <div ref={messageRef} className={`flex mt-2 ${isOwnMessage ? 'justify-end' : 'justify-start'}`}>
      ...
      <div
        className={`rounded-xl px-3 py-2 ${isOwnMessage ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}
      >
        {showHeader && (
          <div className="text-sm font-bold mb-1">{isOwnMessage ? 'You' : 'Other User'}</div>
        )}
        <div>{message}</div>
      </div>
