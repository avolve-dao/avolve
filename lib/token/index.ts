...
export const tokensService = {
  ...
  async getUserBalances(
    userId: string
  ): Promise<{ success: boolean; data?: TokenBalance[]; error?: string }> {
    const supabase = createClient();
  ...
  async spendGen(
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('spend_gen_tokens', {
        p_user_id: userId,
        p_amount: amount,
        p_purpose: purpose,
      });
  ...
  async getTokenSupply(
    tokenSymbol: string
  ): Promise<{ success: boolean; data?: TokenSupplyStats; error?: string }> {
    const supabase = createClient();

    try {
      const { data, error } = await supabase.rpc('get_token_supply_stats', {
