...
type SupabaseQueryHandler<T extends SupabaseTableName> = (query: SupabaseSelectBuilder<T>) => SupabaseSelectBuilder<T>
...
function createStore<TData extends SupabaseTableData<T>, T extends SupabaseTableName>(props: UseInfiniteQueryProps<T>) {
  ...
  const fetchPage = async (skip: number) => {
    ...
    let query = supabase.from(tableName).select(columns, { count: "exact" }) as unknown as SupabaseSelectBuilder<T>
...
function useInfiniteQuery<TData extends SupabaseTableData<T>, T extends SupabaseTableName = SupabaseTableName>(
  ...
  useEffect(() => {
    ...
    if (
      ...
      props.tableName !== storeRef.current.getState().getState().tableName
    ) {
  ...
  }, [props.tableName, state.hasInitialFetch])
