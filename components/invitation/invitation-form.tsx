...
const sacredGeometryClasses = {
  ...
  input:
    'w-full rounded-md border border-slate-300 p-2.5 text-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500',
  select:
    'w-full rounded-md border border-slate-300 p-2.5 text-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500',
  button:
  ...
  tierCard:
    'border rounded-lg p-4 cursor-pointer transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-500',
  ...
  refreshButton:
    'text-indigo-600 hover:text-indigo-800 text-sm flex items-center mt-2 focus:outline-none focus:underline',
  tooltipContainer: 'relative group',
  tooltip:
    'absolute bottom-full left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs rounded py-1 px-2 mb-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-200 w-max z-10',
  srOnly: 'sr-only',
};
...
const getTokenColor = (tokenType: string): string => {
  switch (tokenType) {
    ...
    case 'GEN':
      return 'bg-gradient-to-r from-neutral-400 to-neutral-600'; // Neutral gradient
    default:
      return 'bg-gray-500';
  }
};
...
const InvitationForm: React.FC = () => {
  ...
  const getTierStatusBadge = (tier: InvitationTierAvailability) => {
    if (tier.available) {
      return (
        <span
          className={`${sacredGeometryClasses.tierBadge} ${sacredGeometryClasses.tierAvailableBadge}`}
        >
      ...
      return (
        <span
          className={`${sacredGeometryClasses.tierBadge} ${sacredGeometryClasses.tierLimitBadge}`}
        >
      ...
      return (
        <span
          className={`${sacredGeometryClasses.tierBadge} ${sacredGeometryClasses.tierUnavailableBadge}`}
        >
          <AlertCircle className="w-3 h-3 mr-1" />
          Insufficient Tokens
        </span>
  ...
  return (
    <div className={sacredGeometryClasses.container} aria-labelledby="invitation-form-title">
      <h2 id="invitation-form-title" className={sacredGeometryClasses.header}>
        Create Invitation
      </h2>
      ...
      >
        ...
        <div>
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {tiers.map(tier => (
                  ...
                  >
                    ...
                    <div className="mb-2">{getTierStatusBadge(tier)}</div>
                    ...
                    <div className={sacredGeometryClasses.tierCardDetails}>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span aria-labelledby={`validity-${tier.id}`}>
                          {tier.validity_days} days
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span id={`multiplier-${tier.id}`}>Reward Multiplier:</span>
                        <span aria-labelledby={`multiplier-${tier.id}`}>
                          x{tier.reward_multiplier}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span id={`limit-${tier.id}`}>Monthly Limit:</span>
                        <div className={sacredGeometryClasses.tooltipContainer}>
                          <span aria-labelledby={`limit-${tier.id}`}>
                            {tier.remaining_invites} / {tier.max_invites}
                          </span>
          ...
          <input type="hidden" {...register('tier_name')} aria-hidden="true" />

          {errors.tier_name && (
            <p className={sacredGeometryClasses.errorText} role="alert">
              {errors.tier_name.message}
            </p>
          )}
        </div>
