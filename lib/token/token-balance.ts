...
import { TokenBalance, TokenResult, TokenError } from './token-types';
  ...
  public async getUserTokenBalances(userId: string): Promise<TokenResult<TokenBalance[]>> {
    try {
      ...
      for (const ownership of ownerships) {
        const { data: token, error: tokenError } = await this.repository.getTokenById(
          ownership.token_id
        );
      ...
      return {
        ...
        error: new TokenError(
          'An unexpected error occurred while getting user token balances',
          error
        ),
      };
    }
