...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: stakeResult, error: stakeError } = await (supabase as any).rpc('stake_tokens', {
      p_user_id: user.id,
...
export async function GET() {
  try {
    ...
    if (rulesError) {
      ...
      return NextResponse.json({ error: 'Failed to fetch staking rules' }, { status: 500 });
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { data: unstakeResult, error: unstakeError } = await (supabase as any).rpc(
      'unstake_tokens',
      {
        p_user_id: user.id,
        p_stake_id: stakeId,
    ...
    if (unstakeError) {
      ...
      return NextResponse.json({ error: unstakeError.message }, { status: 500 });
    }
