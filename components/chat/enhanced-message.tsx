...
const EnhancedMessage = memo(function EnhancedMessage({
  ...
  return (
    <div ref={messageRef} className={`flex gap-2 mb-4 ${isCurrentUser ? "justify-end" : "justify-start"}`}>
      {!isCurrentUser && (
      ...
      <div className={`max-w-[70%] ${isCurrentUser ? "items-end" : "items-start"}`}>
        {!isCurrentUser && <div className="text-xs text-muted-foreground mb-1">{message.user.name}</div>}
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent side="top">
                    {readReceipts.length > 0 ? (
                      <div className="text-xs">Read by: {readReceipts.map((r) => r.user.name).join(", ")}</div>
                    ) : (
