...
import { ConsensusService, ConsensusMetingData } from './consensus-service';
...
export function useConsensus(supabaseClient: SupabaseClient, userId?: string) {
  ...
  const consensusService = useMemo(
    () => new ConsensusService(supabaseClient, tokenService),
    [supabaseClient, tokenService]
  );
  ...
  const scheduleMeeting = useCallback(
    async (meetingTime: Date, durationMinutes: number, tokenTypeId: string) => {
  ...
  const getGroupsForMeeting = useCallback(
    async (meetingId: string) => {
      if (!userId) return [];
  ...
  const getParticipantsForGroup = useCallback(
  ...
  const reportConsensusRankings = useCallback(
    async (groupId: string, rankings: Record<string, number>) => {
      if (!userId) return false;
