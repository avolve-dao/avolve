...
export function useNotifications() {
  ...
  const notificationService = useMemo(
    () => new NotificationService(authService.getSupabaseClient()),
    [authService]
  );
    ...
    async (limit: number = 20, offset: number = 0, unreadOnly: boolean = false) => {
      ...
      try {
        ...
        const { data: notifications, error: notificationsError } =
          await notificationService.getUserNotifications(user.id, limit, offset, unreadOnly);
        ...
        // Get unread count
        const { data: unreadCount, error: countError } =
          await notificationService.getUnreadNotificationCount(user.id);
  ...
  const deleteAllReadNotifications = useCallback(async () => {
    ...
    try {
      ...
      setState(prev => ({
        ...
        isLoading: false,
        error:
          error instanceof Error ? error : new Error('Failed to delete all read notifications'),
      }));
      return {
        data: null,
        error:
  ...
  const createTokenNotification = useCallback(
    async (
      userId: string,
      title: string,
      message: string,
  ...
  const createConsensusNotification = useCallback(
    async (
      userId: string,
      title: string,
      message: string,
      ...
      try {
        ...
        setState(prev => ({
          ...
          isLoading: false,
          error:
            error instanceof Error ? error : new Error('Failed to create consensus notification'),
        }));
        return {
          data: null,
          error:
