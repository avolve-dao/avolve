...
export class InvitationService {
   ...
   */
  public async getUserInvitationsWithInvitees(
    userId: string
   ...
   */
  public async getUserReceivedInvitations(
    userId: string
  ): Promise<InvitationResult<InvitationWithCreator[]>> {
  ...
  public async createInvitation(
    ...
    try {
      ...
      if (
        ...
        !request.tier_name ||
        typeof request.tier_name !== 'string' ||
        !request.tier_name.trim()
      ) {
        return {
          data: null,
      ...
      const { data: hasReachedLimit, error: limitError } = await this.hasUserReachedInvitationLimit(
        userId,
        request.tier_name
      );
      ...
      const { data: tier, error: tierError } = await this.repository.getInvitationTierByName(
        request.tier_name
      );
      ...
      if (tier.token_cost > 0) {
        ...
        const { data: tokenObj, error: tokenObjError } = await this.tokenService.getTokenBySymbol(
          tier.token_type
        );
        if (tokenObjError || !tokenObj) {
          return {
            ...
            error: new InvitationError(
              `Token with symbol ${tier.token_type} not found`,
              tokenObjError
            ),
          };
        }
        const { data: tokenBalance, error: tokenError } = await this.tokenService.getUserBalance(
          userId,
          tokenObj.id
        );
        if (tokenError) {
      ...
      const { data: invitation, error: invitationError } = await this.repository.createInvitation(
        userId,
        request.tier_name,
        request.email
      );
  ...
  public async claimInvitationReward(
    ...
    try {
      // Validate invitation
      const { data: invitation, error: invitationError } = await this.repository.getInvitationById(
      ...
      if (invitation.status !== InvitationStatus.ACCEPTED) {
        return {
          data: null,
          error: new InvitationError(
            `Invitation must be accepted before claiming reward. Current status: ${invitation.status}`
          ),
        };
      }
      ...
      const { data: updatedInvitation, error: updateError } =
        await this.repository.updateInvitationStatus(invitation.id, InvitationStatus.CLAIMED);
      ...
      if (updateError) {
        ...
        console.error(
          'Failed to update invitation status after successful token claim:',
          updateError
        );
      }
      ...
      return {
        data: null,
        error: new InvitationError(
          'An unexpected error occurred while claiming invitation reward',
          error
        ),
      };
    }
  ...
  public async getInvitationDashboard(
    userId: string
  ): Promise<InvitationResult<InvitationDashboardData>> {
    try {
      // Get created invitations
      const { data: createdInvitations, error: createdError } =
        await this.repository.getUserInvitationsWithInvitees(userId);
      ...
      const { data: receivedInvitations, error: receivedError } =
        await this.repository.getUserReceivedInvitations(userId);
      ...
      const invitationsAccepted = createdInvitations
        ? createdInvitations.filter(
            inv =>
              inv.status === InvitationStatus.ACCEPTED || inv.status === InvitationStatus.CLAIMED
          ).length
        : 0;
      const invitationsClaimed = createdInvitations
        ? createdInvitations.filter(inv => inv.status === InvitationStatus.CLAIMED).length
        : 0;
      const invitationsPending = createdInvitations
        ? createdInvitations.filter(inv => inv.status === InvitationStatus.PENDING).length
        : 0;
      const invitationsExpired = createdInvitations
        ? createdInvitations.filter(inv => inv.status === InvitationStatus.EXPIRED).length
        : 0;

      // Calculate conversion rate
      const conversionRate =
        invitationsCreated > 0 ? (invitationsAccepted / invitationsCreated) * 100 : 0;
      ...
      return {
        data: null,
        error: new InvitationError(
          'An unexpected error occurred while getting invitation dashboard',
          error
        ),
      };
    }
  ...
  public async isInvitationCodeValid(code: string): Promise<InvitationResult<boolean>> {
    try {
      ...
      return {
        data: null,
        error: new InvitationError(
          'An unexpected error occurred while checking invitation code',
          error
        ),
      };
    }
  ...
  public async hasUserReachedInvitationLimit(
    ...
    try {
      ...
      const { data: tier, error: tierError } =
        await this.repository.getInvitationTierByName(tierName);
      ...
      const { data: count, error: countError } =
        await this.repository.getUserInvitationCountForTier(userId, tierName);
      ...
      return {
        data: null,
        error: new InvitationError(
          'An unexpected error occurred while checking invitation limit',
          error
        ),
      };
    }
   ...
   */
  public async getAvailableInvitationTiers(
    userId: string
  ): Promise<InvitationResult<InvitationTierAvailability[]>> {
    try {
      // Get all tiers
      const { data: tiers, error: tiersError } = await this.repository.getAllInvitationTiers();
      ...
      for (const tier of tiers) {
        ...
        if (tier.token_cost > 0) {
          const { data: balance, error: balanceError } = await this.tokenService.getUserBalance(
            userId,
            tier.token_type
          );
      ...
      return {
        data: null,
        error: new InvitationError(
          'An unexpected error occurred while getting available invitation tiers',
          error
        ),
      };
    }
  ...
  public async validateInvitationCode(
    code: string
  ): Promise<InvitationResult<InvitationValidationResult>> {
    try {
      if (!code || code.trim() === '') {
        return {
          data: {
      ...
      return {
        data: null,
        error: new InvitationError(
          'An unexpected error occurred while validating invitation code',
          error
        ),
      };
    }
