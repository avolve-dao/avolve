...
export function FreeMembershipBanner() {
  ...
  const urgencyLevel = 
    status.remaining_slots <= 10 ? 'high' : 
    status.remaining_slots <= 25 ? 'medium' : 'low'

  return (
    <Card className={`mb-6 border-2 ${
      urgencyLevel === 'high' ? 'border-red-500 bg-red-50 dark:bg-red-950/20' : 
      urgencyLevel === 'medium' ? 'border-amber-500 bg-amber-50 dark:bg-amber-950/20' : 
      ...
      <CardContent className="pt-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <Badge variant={
            ...
            urgencyLevel === 'medium' ? 'outline' : 'default'
          }>
            {status.remaining_slots} spots left
          </Badge>
        </div>
        <p className="text-sm mb-2">
          We're offering <span className="font-bold">free lifetime membership</span> to our first 100 members!
        </p>
