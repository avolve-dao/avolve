...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!validation.data) {
      ...
      const response = new NextResponse('Internal Server Error: Invalid validation result', {
        status: 500,
      });
      return applySecurityHeaders(response);
    }
    ...
    const response = NextResponse.json({ error: 'Error processing your request' }, { status: 500 });
    return applySecurityHeaders(response);
  }
}
